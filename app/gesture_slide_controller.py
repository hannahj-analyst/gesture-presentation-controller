...
from pointer_drawing import (
    PointerDrawingState,
    pointer_drawing_overlay,
    clear_canvas,
    draw_exit_progress_bar,
)
...
def detect_swipe(x_history: list[int]) -> str | None:
    ...
    avg_velocity = abs(movement) / len(deltas)

    if avg_velocity < SWIPE_MIN_VELOCITY:
        return None
...
def get_pinch_distance(hand_landmarks) -> float:
    """
    Return thumb-tip to index-tip distance normalized by wrist to middle-MCP span.
    """
...
class ZoomSmoother:
    ...
    def update(self, target):
        self.value = self.alpha * target + (1 - self.alpha) * self.value
        return self.value
...
while True:
    ...
    fps_value = 0.1 * fps_raw + 0.9 * fps_value
    ...
    if active_mode == "DRAWING":
        swipe_txt = "Swipe: left = undo | right = redo"
        swipe_clr = (0, 200, 140)
    ...
    if hand_landmarks:
        ...
        # Commit stroke on pinch release
        ...
        if (
            active_mode in {"NAVIGATION", "DRAWING"}
            ...
            if stable_label != held_label:
                ...
                if target_mode == "EXIT":
                    ...
                    if target_mode == "CLEAR":
                        ...
                        last_action_text = f"Locked in {active_mode}: hold fist 2s to exit"
    ...
    # Also quit if the user manually closes either OpenCV window
    webcam_visible = cv2.getWindowProperty("Gesture Webcam", cv2.WND_PROP_VISIBLE)
    slide_visible = cv2.getWindowProperty("Gesture Controlled Slides", cv2.WND_PROP_VISIBLE)

    if webcam_visible < 1 or slide_visible < 1:
        print("Window closed")
        break
